...
Для тренировки сети все изображения масштабировали в $$256$$x$$256$$. Препроцессинг — вычитание среднего(*mean*).
...
*  извлечение случайных фрагментов изображений $$224$$x$$224$$ из изображения $$256$$x$$256$$ и горизонтальное отображение этих фрагментов.
...
Обучали на двух GPU GTX $$580$$ (по $$3$$Gb памяти) в течении 6 дней.
...
Сеть организована по тем же принципам что в вышеприведенной статье. Для обучения использованы $$224$$x$$224$$ *RGB* изображения, препроцессинг — вычитаение среднего(*mean*).

Ключевая особенность полученных сетей — использование маленьких сверток $$3$$x$$3$$, что ползволило увеличить глубину до $$16$$-$$19$$ слоев.

Ранее использовались свертки $$5$$x$$5$$ и $$7$$x$$7$$. Согласно статье использование двух сверточных слоев $$3$$x$$3$$ заменяет один слой $$5$$x$$5$$, а использование $$3$$ слоев $$3$$x$$3$$ заменяет один слой $$7$$x$$7$$. Замена одной большой свертки на несколько маленьких позволяет увеличить число нелинейных преобразований и значительно уменьшить количество параметров сети.
...
### Реализация
Сеть собрали на [Caffe](http://caffe.berkeleyvision.org), модифицировав для возможности обучения на нескольких GPU. Тренировка занимала $$2$$-$$3$$ недели на $$4$$х NVIDIA Titan Black.
